...
\begin{equation}
  \frac{d(N'-N)}{dt}=rN-r\left(1-\frac{N}{K}\right)N=N^2/K
\end{equation}
Writing out $N$ for logistic growth, this hazard is
\begin{equation}
  \lambda(t)\equiv\frac{d(N'-N)}{dt}=N_0^2K\left[
...
\begin{equation}
  (N_0K'/r)\left[\frac{e^{t'}}{K'+e^{t'}-1}\right]dt'.
\end{equation}
...
\begin{equation}
  \frac{N_0 K'}{r}\frac{u}{(K'+u)^2}du.
\end{equation}
The constant can go using $u=Ky$ so that $du=Kdy$ to yield
\begin{equation}
  \frac{N_0 K'}{r}\frac{y}{(y+1)^2}dy.
\end{equation}
...
bring back the lost $N_0K'/r$ term in front, and then
\begin{equation}
  \int_{y_0}^{y_1}\lambda(t)dt=\frac{K}{r}\left[
...
\begin{equation}
  \frac{x_a r}{K}+\ln(y_0+1)-\frac{y_0}{y_0+1}=\ln(y_1+1)-\frac{y_1}{y_1+1}.
\end{equation}
