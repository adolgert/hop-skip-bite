...
void FindNewEvent(SegmentIdx sl_idx, SegmentIdx sr_idx, PointIdx p,
  ...
  if (same_points.size()<4) {
    SMVLOG(BOOST_LOG_TRIVIAL(debug) << "FindNewEvent: Shared points");
    return;
  }
  if (Intersect(points[sl.p0], points[sl.p1], points[sr.p0], points[sr.p1])) {
    ...
    if (int_found && intersection > points.at(p)) {
      ...
      SMVLOG(BOOST_LOG_TRIVIAL(debug) << "Segment intersection above or left "
        "of p");
    }
  } else {
    SMVLOG(BOOST_LOG_TRIVIAL(debug)<<"Segments cannot intersect by CCW. "
      <<sl_idx <<" " << sr_idx);
  }
}
...
segment_intersections_sweep(const std::vector<std::pair<double,double>>& inpoints,
  ...
  SMVLOG(BOOST_LOG_TRIVIAL(debug)<< "Copied inputs " << inpoints.size() << ' '
    << inlines.size());
  ...
  SMVLOG(BOOST_LOG_TRIVIAL(debug)<<"Insert all segment endpoints into queue");
  ...
  SMVLOG(BOOST_LOG_TRIVIAL(debug)<<"Starting Q size " << Q.size());
  ...
  while (next_event!=Q.end()) {
    ...
    SMVLOG(BOOST_LOG_TRIVIAL(debug) << "Line segments associated with "
      << p << ": ");
    for (auto ri=r.first; ri!=r.second; ++ri) {
      const QEntry& entry=ri->second;
      SMVLOG(BOOST_LOG_TRIVIAL(debug) << entry.seg << ' ');
    ...
    SMVLOG(BOOST_LOG_TRIVIAL(debug) << "next event p=" << p << " segs=");
    SMVLOG(BOOST_LOG_TRIVIAL(debug)<<"queue length " << Q.size());
    ...
    if (LUC.size()>1) {
      ...
      SMVLOG(BOOST_LOG_TRIVIAL(debug)<< "LUC intersection " << shared_vertex
        << " (" << points[p].x
        << ", " << points[p].y << ") " << LUC.size());
      for (auto ls : LUC) {
    ...
    if (LC.size()>0) {
      ...
      SegmentIdx rough_idx=*LC.begin();
      TEntry tdel{rough_idx, p, IntLoc::None}; // We know p!=sweep_p so safe.
      auto tdel_ins=T.insert(tdel);
      auto tdel_save=tdel_ins.first;
      BOOST_LOG_TRIVIAL(debug)<<"LC.size() inserted segment"
        << rough_idx << " success " <<tdel_ins.second;
      ...
      T.erase(tdel_save);
      if (search_cnt>LC.size()+1) {
        BOOST_LOG_TRIVIAL(info)<<"LC search was "<<search_cnt
        <<" for "<<LC.size();
      }
    }

    // Now we can set the current point for the sort with impunity.
    if (std::floor(points[p].y*100)!=std::floor(points[sweep_p].y*100)) {
      BOOST_LOG_TRIVIAL(info)<<"Sweep algorithm at "<<points[p].y;
    }
    sweep_p=p;
    ...
    // 7. if U+C==0
    SMVLOG(BOOST_LOG_TRIVIAL(debug)<< "UC size " << UC.size());
    if (UC.size()==0) {
      if (bSegDebug) CheckStatus(T, T_sort);
      SMVLOG(BOOST_LOG_TRIVIAL(debug)<< "size(UC)=0");
      ...
      if (probe_iter!=T.begin()) {
        ...
        ++sr;
        if (sr!=T.end()) {
          SMVLOG(BOOST_LOG_TRIVIAL(debug)<< "8. find event " << sl->seg << "-"
            << sr->seg);
          FindNewEvent(sl->seg, sr->seg, p, lines, points, Q);
        } else {
          SMVLOG(BOOST_LOG_TRIVIAL(debug)<< "8. " << p <<
            " has no right neighbor. no search");
        }
      } else {
        SMVLOG(BOOST_LOG_TRIVIAL(debug)<<"8. " << p << " has no left neighbor. "
          "no search");
      }
      T.erase(probe_iter);
    } else {
      SMVLOG(BOOST_LOG_TRIVIAL(debug) << "size(UC) " << UC.size());
      ...
      if (bSegDebug) CheckStatus(T, T_sort);
      SMVLOG(BOOST_LOG_TRIVIAL(debug) << "\tLeast u "<<least_u->seg<<'-'
        <<least_u->pt <<" greatest u " <<greatest_u->seg <<'-'
        <<greatest_u->pt);
      if (least_u!=T.begin()) {
        auto sl=least_u;
        --sl;
        SMVLOG(BOOST_LOG_TRIVIAL(debug)<< "8. Find event " << sl->seg << "-" <<
          least_u->seg);
        FindNewEvent(sl->seg, least_u->seg, p, lines, points, Q);
      } else {
        SMVLOG(BOOST_LOG_TRIVIAL(debug)<< "8. Least_u is at beginning. "
          "No search.");
      }
      auto sr=greatest_u;
      ++sr;
      if (sr!=T.end()) {
        FindNewEvent(greatest_u->seg, sr->seg, p, lines, points, Q);
      } else {
        SMVLOG(BOOST_LOG_TRIVIAL(debug)<<"8. Greatest_u is at end. No search.");
      }
    }
      ...
      if (p0_idx!=p2_idx && p0_idx!=p3_idx && p1_idx!=p2_idx && p1_idx!=p3_idx){
        if (Intersect(p0, p1, p2, p3)) {
          ...
          if (found) {
            ...
            SMVLOG(BOOST_LOG_TRIVIAL(debug) << "Couldn't find intersection");
          }
        }
      }
    }
...
segment_intersections_ab(const std::vector<std::pair<double,double>>& inpoints,
  ...
  for (size_t s0idx=0; s0idx<inlinesa.size(); ++s0idx) {
    ...
    Point p0(inpoints[p0_idx]);
    Point p1(inpoints[p1_idx]);
    for (size_t s1idx=0; s1idx<inlinesb.size(); ++s1idx) {
      size_t p2_idx=inlinesb[s1idx].first;
      size_t p3_idx=inlinesb[s1idx].second;
      Point p2(inpoints[p2_idx]);
      Point p3(inpoints[p3_idx]);
      if (Intersect(p0, p1, p2, p3)) {
